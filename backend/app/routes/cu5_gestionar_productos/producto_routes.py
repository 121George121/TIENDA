...
# Lectura pública: la app móvil y el catálogo web muestran productos sin sesión iniciada.
...
# Mutaciones: solo el panel admin las usa, requieren sesión iniciada.
@router.post("", response_model=ProductoResponse, status_code=status.HTTP_201_CREATED)
def crear_producto(data: ProductoCreate, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)):
    return ProductoController.crear_producto(db=db, data=data)

@router.put("/{id}", response_model=ProductoResponse)
def actualizar_producto(id: int, data: ProductoUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)):
...
def cambiar_estado_producto(id: int, activo: bool = Query(...), db: Session = Depends(get_db), current_user=Depends(get_current_active_user)):
    return ProductoController.cambiar_estado(db=db, id=id, activo=activo)

@router.delete("/{id}")
def baja_logica_producto(id: int, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)):
